...
| Phát hành token | OpenIddict (OIDC server tự host, phát JWKS) | Có |
| Verify token | JWT Bearer middleware | Có |
| Phân quyền theo project | Logic nghiệp vụ ở Workspace Service | Không — tự xây |

**Quyết định IdP (đã chốt): OpenIddict, federation-ready.** Identity Service tự host một OIDC server bằng OpenIddict, phát RS256 JWT kèm JWKS chuẩn (`/.well-known/openid-configuration`). Gateway và mọi service validate token qua authority **cấu hình được** — nên khi lên AWS có thể federate sang **AWS Cognito** mà không sửa business code, chỉ đổi biến config. Chọn OpenIddict thay vì Cognito-làm-IdP để giữ DX local-first (chạy offline trong Aspire/Docker, không cần AWS credential ngày đầu) và thể hiện năng lực tự dựng OIDC.

> **Lưu ý chống nhầm — "AWS IAM" ≠ login của user app.** AWS IAM dùng để phân quyền **tài nguyên hạ tầng cloud** (IAM Role cho EKS/ECS qua IRSA, quyền Execution ghi S3, Secrets Manager giữ secret) — chuyện deploy/CI. Danh tính người dùng app (đăng nhập, JWT, OAuth) là việc của Identity Service (OpenIddict), hoàn toàn tách biệt.
...
| Production | Kubernetes (AWS EKS) | Tự scale, health check, rolling update, managed services |
...
- Cloud chính (đã chốt): **AWS** — EKS (Kubernetes) + ECR (registry) + RDS for PostgreSQL + S3 (artifact Execution, thay MinIO ở prod) + Secrets Manager. Cognito để dành cho phương án federation IdP (xem mục 3.1).
- Domain quraex.com chỉ trỏ vào MỘT điểm: API Gateway (Kong) và frontend. Các service nội bộ KHÔNG lộ ra internet — chỉ Gateway có public endpoint.
- TLS/HTTPS kết thúc ở ingress của Kubernetes (hoặc ở Gateway). Đây là ranh giới bảo mật quan trọng.
- Database, Redis, RabbitMQ nên dùng managed service của AWS (RDS for PostgreSQL, ElastiCache for Redis, Amazon MQ for RabbitMQ) thay vì tự host trong cluster — giảm gánh vận hành.
- Quản lý cấu hình & secret: dùng Kubernetes Secret + **AWS Secrets Manager**; quyền truy cập tài nguyên cấp qua **IAM Role (IRSA)**, không hard-code trong image hay repo.
