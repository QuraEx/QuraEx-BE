...
Compose tách 2 file theo trách nhiệm: `docker-compose.yml` (chỉ backing infra — Postgres/RabbitMQ/Redis) + `docker-compose.app.yml` (overlay thêm `gateway` + `authoring`). `make up` gộp cả hai. Stack chạy ở môi trường `Development` để khớp Aspire: migration tự apply, dùng JWT public key dev đã commit. Đây là local/CI parity — production dùng secret thật + migration job riêng.
...
**Đặc điểm Dockerfile đã hiện thực** (`gateway/QuraEx.Gateway/Dockerfile`, `services/authoring/QuraEx.Authoring.Api/Dockerfile`):
