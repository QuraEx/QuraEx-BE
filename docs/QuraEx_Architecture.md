...
- API Gateway (YARP): điểm vào duy nhất, định tuyến, xác thực biên, rate-limit.
...
| --- | --- | --- |
| Dev (máy lập trình viên) | .NET Aspire AppHost | 1 lệnh chạy cả 8 service + Postgres + RabbitMQ + Redis, kèm dashboard observability |
| Chạy chung / demo | Docker Compose | Môi trường nhẹ, đồng nhất cho cả nhóm và CI |
...
Dockerfile mẫu (multi-stage cho một service .NET):

# build stage  
FROM mcr.microsoft.com/dotnet/sdk:8.0 AS build  
WORKDIR /src  
COPY ["TestArtifact.Api/TestArtifact.Api.csproj", "TestArtifact.Api/"]  
RUN dotnet restore "TestArtifact.Api/TestArtifact.Api.csproj"  
COPY . .  
RUN dotnet publish "TestArtifact.Api/TestArtifact.Api.csproj" \  
 -c Release -o /app/publish  

# runtime stage (nhỏ gọn)  
FROM mcr.microsoft.com/dotnet/aspnet:8.0 AS final  
WORKDIR /app  
COPY --from=build /app/publish .  
EXPOSE 8080  
ENTRYPOINT ["dotnet", "TestArtifact.Api.dll"]

## 10.3. CI/CD — GitHub Actions

Pipeline chạy khi push hoặc pull request: build → test → đóng gói image → đẩy registry → deploy. Integration test dùng Testcontainers (spin DB/broker thật trong container lúc test rồi xoá), không mock.

Workflow mẫu (rút gọn):
...
jobs:  
 ...
 - run: dotnet test --no-build # gồm Testcontainers  
 ...
 with:  
 registry: quraex.azurecr.io  
 username: ${{ secrets.ACR\_USER }}  
 password: ${{ secrets.ACR\_PASS }}  
 - run: |  
 docker build -t quraex.azurecr.io/testartifact:${{ github.sha }} \  
 -f TestArtifact.Api/Dockerfile .  
 docker push quraex.azurecr.io/testartifact:${{ github.sha }}  
 ...
 with:  
 manifests: k8s/testartifact.yaml  
 images: quraex.azurecr.io/testartifact:${{ github.sha }}
