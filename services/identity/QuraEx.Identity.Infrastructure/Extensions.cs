...
namespace QuraEx.Identity.Infrastructure;
...
public static class Extensions
{
        ...
        IHostEnvironment env)
    {
        ...
        {
            options.UseNpgsql(configuration.GetConnectionString("postgres-identity"),
...
internal sealed class MigrationHostedService(IServiceScopeFactory scopeFactory) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await db.Database.ExecuteSqlRawAsync(
            "SELECT pg_advisory_lock(hashtext('identity_migration'))", stoppingToken);
        try
        {
        ...
        {
            await db.Database.ExecuteSqlRawAsync(
                "SELECT pg_advisory_unlock(hashtext('identity_migration'))", stoppingToken);
        }
    }
}
